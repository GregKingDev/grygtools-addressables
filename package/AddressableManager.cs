...
namespace GrygTools.AddressableUtils
{
	public class AddressableManager : SingletonBase<AddressableManager>, ISingleton
	{
		...
		public async Task<(bool, T)> TryLoadAssetReferenceAsync<T>(AssetReference assetReference) where T : class
		{
			T tObject = await LoadAssetReferenceAsync<T>(assetReference);
			return (tObject != null, tObject);
		}
		...
		{
			return LoadAssetReferenceAsync<T>(assetReference).Result;
		}

		public T LoadAssetFromLocation<T>(string location) where T : class
		{
			return LoadAssetFromLocationAsync<T>(location).Result;
		}
		...
		public async Task<IEnumerable<object>> LoadLabel(string label)
		{
			...
			if (handle.Status == AsyncOperationStatus.Succeeded)
			{
				...
				return tasks.Select(a => a.Result);
			}
			Debug.LogError($"Error loading label: {label}");
			return null;
		}
	}
}
